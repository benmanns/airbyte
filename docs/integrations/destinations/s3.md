...
4. Configure fields:
   ...
   - **Use a Staging Directory**
     - If enabled, the connector will stage new data in a temporary directory before deleting
       old data (if applicable), then move the new data to the final directory. This can ensure
       ...
       become costly for buckets with a large number of objects.
   - **S3 Staging Prefix**
     - The prefix to use for the staging directory. If not provided, Airbyte will use
       `{Your "S3 Bucket Path"}/__airbyte_tmp/`.
   - **S3 Endpoint**
